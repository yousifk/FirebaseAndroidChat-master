package yk.firebaseandroid.adapter;
...
import yk.firebaseandroid.R;
import yk.firebaseandroid.model.ChatModel;

/**
 * Created by ykBarreto on 23/06/2016.
 */
public class ChatFirebaseAdapter extends FirebaseRecyclerAdapter<ChatModel,ChatFirebaseAdapter.MyChatViewHolder> {
    ...
    protected void populateViewHolder(MyChatViewHolder viewHolder, ChatModel model, int position) {
        ...
        if (model.getFile() != null){
            ...
            viewHolder.setIvChatPhoto(yk.firebaseandroid.util.Util.local(model.getMapModel().getLatitude(),model.getMapModel().getLongitude()));
